...
class BismuthConfig(BismuthBase):
    ...
    _vars = {
        ...
        "jsonrpc_listen": [
            "bool",
            True,
        ],  # Should the node open it's json-rpc listening interface?
        # integrated wallet server
        "walletserver_port": ["int", 8150],
        "walletserver_listen": [
            "bool",
            True,
        ],  # Should the node open it's wallet server listening interface?
        # Log related
        ...
        for key, default in self._vars.items():
            if key not in self.__dict__:
                ...
                self.app_log.warning(
                    f"Config: Trying to redefine protected key '{key}'."
                )
    ...
    def get(self, key: str):
        ...
        if key not in self._vars:
            ...
            if self.log_debug:
                ...
                self.app_log.debug(
                    f"Config: Type '{exc_type}' fname '{fname}' line {exc_tb.tb_lineno}."
                )
            return
